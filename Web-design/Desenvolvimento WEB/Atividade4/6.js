// porcentagem
function percentOf(val1, val2)
{
    var percent;
    
    percent = val1 / val2;
    percent = percent * 100;
    
    return percent;
}

// metade do numero
function halfNumber(val)
{
    var div;
    
    div = val / 2;
    
    return div;
}

// quadrado
function squareNumber(val)
{
    var potc;
    
    potc = Math.pow(val, 2);
    
    return potc;
}

// area do círculo
function areaOfCircle(raio)
{
    var area;
    
    area = Math.PI * (Math.pow(raio, 2));
    
    return area;
}

// Função responsável por chamar as outras funções
function operations(val)
{
    var resul;
    
    	resul = halfNumber(val);
    	alert("Sua metade é: " +resul);
    	resul = squareNumber(resul);
	alert("Seu quadrado é: " +resul);
	resul = areaOfCircle(resul);
	alert("Sua área é de: " +resul.toFixed(2));
    	resul = percentOf(resul, 2);
    	alert("Sua porcentagem é de: " +resul.toFixed(2)+ "%");
}

// Função principal
function main()
{
	var camp1 = document.getElementById("camp");
	operations(camp1.value);
}